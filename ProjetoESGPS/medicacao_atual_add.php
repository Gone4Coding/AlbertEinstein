...
$tituloPagina = "Adicionar Medicação Atual";
...
if (!empty($_POST)) {
    ...
    if (count($msgErros) > 0) {
        ...
        $novoMedInserido = addMedAtual($data["data_comeco"],$data["motivo"],$data["idmedicamento"], $num_utente);

        if ($novoMedInserido != -1) {
            $_SESSION["flash_msgGlobal"] = "A Medicação foi inserido com sucesso";
            $_SESSION["flash_tipoMsgGlobal"] = "S";
            header("Location: medicacao_atual_add.php");
            exit;
        } else {
...
require("view/medicacao_atual_add.view.php");
